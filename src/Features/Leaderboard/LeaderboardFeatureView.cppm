...
void draw(const LeaderboardFeature::State &state) {
  ...
  const std::string subtitle =
      std::format("{}x{}   (L to close)", state.gridSize, state.gridSize);
  DrawText(subtitle.c_str(), 16, 44, 16, GRAY);

  int y = 78;
  if (state.merged.empty()) {
    DrawText(state.isLoadingRemote || state.isLoadingLocal ? "Loading…"
                                                           : "No scores yet",
             16, y, 18, GRAY);
  } else {
    int rank = 1;
    for (const auto &entry : state.merged) {
      ...
      const std::string line =
          std::format("{:>2}. {:<10} {:02}:{:02}  {} mv", rank, entry.name,
