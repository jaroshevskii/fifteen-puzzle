...
// `withDependencies` (override for a closure) and `prepareDependencies` (set the
// global defaults at app launch).
...
class DependencyValues {
 public:
  ...
  typename Key::Value& get() {
    const auto index = std::type_index(typeid(Key));
    auto it = storage_.find(index);
    if (it == storage_.end()) {
      it = storage_.emplace(index, std::make_shared<Model<typename Key::Value>>(resolveDefault<Key>())).first;
    }
  ...
  template <typename Key>
  void set(typename Key::Value value) {
    storage_[std::type_index(typeid(Key))] = std::make_shared<Model<typename Key::Value>>(std::move(value));
  }
 ...
 private:
  ...
  template <typename Key>
  typename Key::Value resolveDefault() {
    switch (context) {
...
// Overrides dependencies for the duration of `operation`, restoring the previous
// scope afterwards (even if `operation` throws).
...
// Sets dependency values globally, intended to be called as early as possible at
// app launch. Equivalent to TCA's `prepareDependencies`.
