...
// A C++ port of TCA 2.0's root `Store`. It owns the single source-of-truth state
// and runs the feature. State is mutated only on the store's main thread:
...
class RootStore final : public Store<State, Action> {
 public:
  ...
  RootStore(State initial, FeatureFactory makeFeature)
      : state_(std::move(initial)), feature_(makeFeature()), mainThread_(std::this_thread::get_id()) {
    std::lock_guard lock(stateMutex_);
    feature_.mount(state_, *this);  // onMount — like TCA 2.0, runs once at creation
  }
  ...
  void addTask(std::function<void(Store<State, Action>&, std::stop_token)> work, std::string cancelID = {}) override {
    std::lock_guard lock(tasksMutex_);
    if (cancelID.empty()) {
      ...
      anonymousTasks_.push_back(AnonymousTask{.done = std::move(done), .thread = std::move(worker)});
    } else {
      // Replacing an in-flight task with the same id requests its stop and joins.
      cancellableTasks_[cancelID] =
          std::jthread([this, work = std::move(work)](std::stop_token token) { work(*this, token); });
    }
  }
