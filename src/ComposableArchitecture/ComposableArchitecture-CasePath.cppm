...
// `Part` out of a `Whole` (when present) and embed a `Part` back into a `Whole`.
...
// Builds a case path for a `std::variant` `Whole` whose `Case` alternative wraps
// a `Part` in a single member.
