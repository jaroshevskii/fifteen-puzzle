...
auto findEmpty = [](const std::vector<Card>& cards) -> std::ptrdiff_t {
  auto it = std::find_if(cards.begin(), cards.end(), [](const Card& c){ return c.text.empty(); });
...
State reducer(const State& s, const Action& a) {
  ...
  std::visit([&](auto&& act){
    ...
    if constexpr (std::is_same_v<T, Shuffle>) ns.cards = shuffleTexts(ns.cards);
      ...
      auto empty = findEmpty(ns.cards);
      if (empty != -1 && adjacent(act.index, static_cast<int>(empty)))
...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    if (!s.isEnd) {
      ...
      auto empty = findEmpty(s.cards);
      int r = static_cast<int>(empty) / GRID;
      int c = static_cast<int>(empty) % GRID;
